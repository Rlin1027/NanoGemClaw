...
export function createGroupsRouter(deps: GroupsRouterDeps): Router {
  ...
  const { validateFolder, validateNumericParam, emitDashboardEvent } = deps;
  ...
  router.get('/groups/:folder/detail', async (req, res) => {
    ...
    try {
      ...
      const group = groups.find((g) => g.id === folder || g.folder === folder);
      if (!group) {
  ...
  router.post('/personas', async (req, res) => {
    try {
      ...
      if (!key || !name || !systemPrompt) {
        res
          .status(400)
          .json({ error: 'Missing required fields: key, name, systemPrompt' });
        return;
      }
      if (!SAFE_FOLDER_RE.test(key)) {
        res.status(400).json({
          error: 'Invalid persona key (alphanumeric, dash, underscore only)',
        });
        return;
      }
  ...
  router.put('/groups/:folder/preferences', async (req, res) => {
    ...
    if (!ALLOWED_KEYS.includes(key)) {
      res
        .status(400)
        .json({ error: `Invalid key. Allowed: ${ALLOWED_KEYS.join(', ')}` });
      return;
    }
    try {
      const db = await import('../db.js');
  ...
  router.get('/search', async (req, res) => {
    try {
      ...
      let offset = 0;
      if (req.query.offset) {
        const parsedOffset = validateNumericParam(
