...
| **아키텍처**          | 모놀리식             | 모듈형 모노레포 (8 패키지 + 7 플러그인)                       |
...
- **모듈형 모노레포** - 8개의 npm 워크스페이스 패키지. 개별 패키지를 자체 프로젝트에 사용하거나 전체 스택을 배포할 수 있습니다.
...
- **음성 인식** - 음성 메시지를 Gemini 멀티모달로 자동 변환 (기본, FFmpeg 불필요) 또는 Google Cloud Speech.
...
- **웹 대시보드** - 12개 모듈 실시간 커맨드 센터（로그 스트리밍, 메모리 편집기, 분석, Google 계정 관리, Drive 브라우저, Discord 설정 포함）.
- **i18n** - 8개 언어 전체 인터페이스 지원: 영어, 번체 중국어, 간체 중국어, 일본어, 한국어, 스페인어, 포르투갈어, 러시아어.
...
| `@nanogemclaw/event-bus`  | 플러그인 간 통신을 위한 타입화된 pub/sub 이벤트 시스템 | 중간        |
...
| **FFmpeg**      | GCP STT 전용 (선택) | `brew install ffmpeg`               |
...
NanoGemClaw는 `plugins/` 디렉토리에 7개의 내장 플러그인을 포함하고 있습니다:
...
| **memorization-service**    | Event Bus를 통한 자동 대화 요약          |             |  이벤트 기반  |
...
| `ALERTS_ENABLED`     | `true`                     | 메인 그룹에 오류 알림 활성화    |
| `CONTAINER_MAX_OUTPUT_SIZE` | `10485760`          | 컨테이너 최대 출력 크기 (바이트) |
| `SCHEDULER_CONCURRENCY` | auto                    | 최대 동시 스케줄 컨테이너 수    |
| `BACKUP_RETENTION_DAYS` | `7`                     | 데이터베이스 백업 보관 일수     |
| `HEALTH_CHECK_ENABLED` | `true`                   | 헬스 체크 HTTP 서버 활성화     |
| `HEALTH_CHECK_PORT`  | `8080`                     | 헬스 체크 서버 포트             |
...
- `/admin help` — 사용 가능한 관리 명령 표시
- `/admin stats` — 가동 시간, 메모리 사용량, 토큰 통계 표시
- `/admin groups` — 등록된 모든 그룹 및 상태 목록
- `/admin tasks` — 모든 예약 작업 목록
- `/admin errors` — 최근 오류가 발생한 그룹 표시
- `/admin report` — 일일 사용 보고서 생성
- `/admin language <lang>` — 봇 인터페이스 언어 변경
- `/admin persona <name|list|set>` — 봇 페르소나 관리
- `/admin trigger <group> <on|off>` — @멘션 트리거 요구 사항 토글
...
| `@nanogemclaw/event-bus`  | `EventBus`, `NanoEventMap`, 타입화된 pub/sub 싱글턴                                          |
...
- **백업** (`store/backups/`): 자동 일일 SQLite 백업, 보관 기간 설정 가능 (`BACKUP_RETENTION_DAYS`)

### 헬스 체크

`HEALTH_CHECK_PORT` (기본 8080)에서 경량 HTTP 서버 실행:

- `GET /health` — 시스템 헬스 상태 (healthy/degraded/unhealthy)
- `GET /ready` — 오케스트레이터용 레디니스 프로브
- `GET /metrics` — Prometheus 형식 메트릭

`HEALTH_CHECK_ENABLED=false`로 비활성화.
...
npm test                  # 모든 테스트 실행 (Vitest, 41 파일, ~950 테스트)
...
- **STT가 실패하나요?** 기본 제공자(`gemini`)는 추가 의존성이 필요하지 않습니다. `STT_PROVIDER=gcp` 사용 시 `ffmpeg`를 설치하세요 (`brew install ffmpeg`).
