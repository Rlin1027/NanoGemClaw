...
import type { NanoPlugin, PluginApi, GeminiToolContribution, RouteContribution } from '@nanogemclaw/plugin-api';
...
export { searchFiles, getFileContent, listFolderContents } from './drive-api.js';
...
const geminiTools: GeminiToolContribution[] = [
            ...
            try {
                ...
                const header =
                    ...
                    (meta.modifiedTime
                        ...
                        : '') +
                    (extracted.truncated ? '_[Content truncated to 100,000 characters]_\n' : '') +
                    '\n---\n\n';
...

    {
        ...
        async execute(args, context): Promise<string> {
            // Delegate to read_file_content — same behavior, different framing for the model
            const readTool = geminiTools.find((t) => t.name === 'read_file_content')!;
            return readTool.execute(args, context);
        },
    },
];
...
const googleDrivePlugin: NanoPlugin = {
    ...
    name: 'Google Drive',
    version: '0.1.0',
    description: 'Search and read Google Drive files from Gemini and the dashboard',
