...
export interface DailyReport {
    ...
    usage: { total_requests: number; avg_duration_ms: number; total_tokens: number };
...
function truncateEmbed(embed: DiscordEmbed): DiscordEmbed {
    ...
    while (result.fields.length > 0 && embedCharCount(result) > DISCORD_EMBED_CHAR_LIMIT) {
        result.fields.pop();
    }

    // Append truncation notice to description
...
export function formatDailyEmbed(report: DailyReport): DiscordPayload {
    const color = pickColor(report.errors.groups_with_errors, report.errors.total_failures);

    const topGroupsValue =
        report.top_groups.length > 0
            ? report.top_groups
                  .map((g, i) => `${i + 1}. **${g.name}** — ${fmtNumber(g.requests)} req`)
                  .join('\n')
...
export function formatWeeklyEmbed(weeklyData: WeeklyData): DiscordPayload {
    ...
    const errorColor = weeklyData.errorRate > 0.05 ? COLOR_RED : weeklyData.errorRate > 0.01 ? COLOR_YELLOW : COLOR_BLUE;
...
export function formatTestEmbed(): DiscordPayload {
    const embed: DiscordEmbed = {
        ...
        description: 'Webhook is configured correctly. Reports will be delivered here.',
        color: COLOR_GREEN,
        fields: [],
