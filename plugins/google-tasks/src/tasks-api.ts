...
export async function listTaskLists(): Promise<TaskList[]> {
    ...
    return items.map((item) => normalizeTaskList(item as Record<string, unknown>));
}
...
export async function getDefaultTaskList(): Promise<TaskList> {
    ...
    const items = res.data.items ?? [];
    if (items.length === 0) {
        throw new Error('Google Tasks: no task lists found for this account');
    }
    // The first list returned is typically the default/primary list
    return normalizeTaskList(items[0] as Record<string, unknown>);
}
 ...
 */
export async function completeTask(tasklistId: string, taskId: string): Promise<Task> {
    const tasks = getAuthenticatedClient();
 ...
 */
export async function findTaskListByName(name: string): Promise<TaskList | null> {
