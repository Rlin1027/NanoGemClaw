        ...
        async execute(args, context): Promise<string> {
            if (!isAuthenticated()) {
                return JSON.stringify({
                    error: 'Not authenticated with Google. Authorize via dashboard Settings → Google Account.',
                });
            }
            ...
            try {
                ...
                pluginApi?.logger.error(`Google Tasks create_google_task error: ${err}`);
                return JSON.stringify({ error: 'Failed to create task in Google Tasks.' });
            }
        },
    },

    {
        ...
        parameters: {
            ...
            properties: {
                ...
                tasklist: {
                    type: 'STRING',
                    description: 'Task list name to search in (default: search all lists)',
                },
            },
            required: ['title'],
        },
        ...
        async execute(args, _context): Promise<string> {
            ...
            try {
                ...
                const found = await findGoogleTaskByTitle(
                    title,
                    pluginApi?.logger ?? { info: () => {}, warn: () => {}, error: () => {}, debug: () => {} },
                );
                if (!found) {
                    return JSON.stringify({
                ...
                pluginApi?.logger.error(`Google Tasks complete_google_task error: ${err}`);
                return JSON.stringify({ error: 'Failed to complete task in Google Tasks.' });
            }
        },
    },

    {
        ...
        async execute(args, _context): Promise<string> {
            ...
            try {
                ...
                return JSON.stringify({ error: 'Failed to list tasks from Google Tasks.' });
            }
        },
    },
];
...
const ipcHandlers: IpcHandlerContribution[] = [
    {
        ...
        async handle(data, _context): Promise<void> {
            if (!pluginApi) return;
            if (!isAuthenticated()) {
                pluginApi.logger.warn('Google Tasks IPC google_task_create: not authenticated');
                return;
            }

            const title = String(data['title'] ?? '');
            if (!title) {
                pluginApi.logger.warn('Google Tasks IPC google_task_create: missing title');
                return;
            }
            ...
            try {
                ...
                pluginApi.logger.error(`Google Tasks IPC google_task_create error: ${err}`);
            }
        },
    },

    {
        ...
        async handle(data, _context): Promise<void> {
            if (!pluginApi) return;
            if (!isAuthenticated()) {
                pluginApi.logger.warn('Google Tasks IPC google_task_complete: not authenticated');
                return;
            }
            ...
            try {
                if (localId) {
                    await markGoogleTaskComplete(pluginApi.dataDir, localId, pluginApi.logger);
                } else if (title) {
                ...
                pluginApi.logger.error(`Google Tasks IPC google_task_complete error: ${err}`);
            }
        },
    },
];
    ...
    createRouter(): Router {
        ...
        router.get('/lists/:listId/tasks', async (req, res) => {
            ...
            try {
                ...
                const tasks = await listTasks(req.params['listId'], {
                    showCompleted,
                    dueMin,
                    dueMax,
                });
                ...
                pluginApi?.logger.error(`Google Tasks route GET /lists/:listId/tasks error: ${err}`);
        ...
        router.post('/lists/:listId/tasks', async (req, res) => {
            ...
            try {
                const task = await createTask(req.params['listId'], {
                    title,
                ...
                pluginApi?.logger.error(`Google Tasks route POST /lists/:listId/tasks error: ${err}`);
        ...
        router.get('/sync-state', (req, res) => {
            ...
            try {
                ...
                pluginApi.logger.error(`Google Tasks route GET /sync-state error: ${err}`);
    ...
    hooks: {
        ...
        async afterMessage(
            ...
            while ((match = SENTINEL_RE.exec(context.reply)) !== null) {
                ...
                if (localId) {
                    await markGoogleTaskComplete(pluginApi.dataDir, localId, pluginApi.logger).catch(
                        (err) =>
                            pluginApi?.logger.warn(
