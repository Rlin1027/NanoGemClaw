 ...
 */
export function upsertTaskMapping(
    dataDir: string,
    mapping: TaskMapping,
): void {
    const state = loadSyncState(dataDir);
    const idx = state.taskMappings.findIndex((m) => m.localId === mapping.localId);
    if (idx >= 0) {
...
export async function syncTasks(api: PluginApi): Promise<void> {
    ...
    try {
        ...
        api.logger.info(`Google Tasks: sync complete — ${lists.length} list(s) checked`);
    } catch (err) {
...
export async function markGoogleTaskComplete(
    ...
    try {
        ...
        logger.warn(`Google Tasks: failed to complete remote task "${localTaskId}" — ${err}`);
    }
}
...
export async function findGoogleTaskByTitle(
    ...
    try {
        ...
        for (const list of lists) {
            const tasks = await listTasks(list.id, { showCompleted: false });
            const match = tasks.find((t) => t.title.toLowerCase() === lower);
            if (match) {
                return { task: match, listId: list.id };
            }
        }
    } catch (err) {
