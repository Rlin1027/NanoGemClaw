    ...
    async execute(args: Record<string, unknown>, context: ToolExecutionContext): Promise<string> {
        if (!isAuthenticated()) {
            return JSON.stringify({
                error: 'Google Calendar not authorized. Please connect your Google account in Settings.',
            });
        }
        ...
        try {
            ...
            return JSON.stringify({ error: `Failed to create event: ${err}` });
        }
    },
};
    ...
    async execute(args: Record<string, unknown>): Promise<string> {
        ...
        try {
            const events = await listEvents({
                timeMin: args.time_min !== undefined ? String(args.time_min) : undefined,
                timeMax: args.time_max !== undefined ? String(args.time_max) : undefined,
                maxResults:
            ...
            return JSON.stringify({ error: `Failed to list events: ${err}` });
        }
    },
};
    ...
    async execute(args: Record<string, unknown>): Promise<string> {
        ...
        try {
            const event = await updateEvent(String(args.event_id), {
                ...
                startTime: args.start_time !== undefined ? String(args.start_time) : undefined,
                endTime: args.end_time !== undefined ? String(args.end_time) : undefined,
                location: args.location !== undefined ? String(args.location) : undefined,
                description: args.description !== undefined ? String(args.description) : undefined,
            });
            ...
            return JSON.stringify({ error: `Failed to update event: ${err}` });
        }
    },
};
...
const deleteCalendarEventTool: GeminiToolContribution = {
    ...
    async execute(args: Record<string, unknown>): Promise<string> {
        ...
        try {
            ...
            return JSON.stringify({ error: `Failed to delete event: ${err}` });
        }
    },
};
...
const checkAvailabilityTool: GeminiToolContribution = {
    ...
    async execute(args: Record<string, unknown>): Promise<string> {
        ...
        try {
            ...
            return JSON.stringify({ error: `Failed to check availability: ${err}` });
        }
    },
};
...
const createCalendarEventIpc: IpcHandlerContribution = {
    ...
    async handle(data: Record<string, unknown>): Promise<void> {
        ...
        if (
            ...
            throw new Error('create_calendar_event IPC requires summary, start_time, end_time');
        }
...
function createEventsRouter(): Router {
    ...
    router.get('/', async (req, res) => {
        if (!isAuthenticated()) {
            res.status(503).json({
                error: 'Google Calendar not authorized. Connect via Settings → Google Account.',
            });
            return;
        }
    ...
    router.get('/today', async (_req, res) => {
        ...
        try {
            ...
            const events = await listEvents({ timeMin: startOfDay, timeMax: endOfDay });
            res.json({ data: events });
        } catch {
