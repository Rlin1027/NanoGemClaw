...
async function runScan(): Promise<void> {
    ...
    if (!isAuthenticated()) {
        pluginApi?.logger.debug('drive-knowledge-rag: skipping scan — Google not authenticated');
        return;
    }
    if (!pluginApi) return;
...
function createRouter(): Router {
    ...
    router.put('/config', async (req, res) => {
        ...
        if (
            ...
            (typeof body.scanIntervalMinutes !== 'number' || body.scanIntervalMinutes < 1)
        ) {
            res.status(400).json({ error: 'scanIntervalMinutes must be a positive number' });
            return;
        }
        if (
        ...
        if (body.scanIntervalMinutes !== undefined && scanTimer !== null && pluginApi) {
            clearInterval(scanTimer);
            const intervalMs = ragConfig.scanIntervalMinutes * 60 * 1000;
            scanTimer = setInterval(() => { void runScan(); }, intervalMs);
        }
...
const plugin: NanoPlugin = {
    ...
    async init(api: PluginApi): Promise<void | false> {
        ...
        // Check Google Auth dependency
        try {
            // isAuthenticated is a synchronous check; if the import itself fails
            // it means the google-auth plugin is absent.
            isAuthenticated();
        } catch {
            api.logger.warn(
                'drive-knowledge-rag: nanogemclaw-plugin-google-auth not available — plugin disabled',
            );
            return false;
        }
        ...
        try {
            ...
            api.logger.warn('drive-knowledge-rag: failed to load config, using defaults');
        ...
        try {
            ...
            api.logger.warn('drive-knowledge-rag: failed to load index, starting fresh');
            knowledgeIndex = emptyIndex();
        }
    ...
    geminiTools: [
        {
            ...
            async execute(args, _context): Promise<string> {
                ...
                const formatted = results
                    .map((r, i) => {
                        const sourceLabel = r.source === 'index' ? 'indexed' : 'live search';
                        const scoreLabel =
