...
| **アーキテクチャ**         | モノリス             | モジュラーモノレポ (8 パッケージ + 7 プラグイン)                                 |
...
- **モジュラーモノレポ** - 8 つの npm ワークスペースパッケージ。個別パッケージを自分のプロジェクトで使用することも、フルスタックをデプロイすることもできます。
...
- **音声認識 (STT)** - 音声メッセージを Gemini マルチモーダルで自動文字起こし（デフォルト、FFmpeg 不要）または Google Cloud Speech。
...
- **Web ダッシュボード** - 12 モジュールのリアルタイム管理センター（ログストリーミング・メモリエディタ・アナリティクス・Google アカウント管理・Drive ブラウザ・Discord 設定を含む）。
- **i18n** - 8 言語の完全なインターフェース対応：英語・繁体中国語・簡体中国語・日本語・韓国語・スペイン語・ポルトガル語・ロシア語。
...
| `@nanogemclaw/event-bus`  | プラグイン間通信用の型付き pub/sub イベントシステム                 | 中         |
...
| **FFmpeg**      | GCP STT のみ (任意) | `brew install ffmpeg`               |
...
- **ダッシュボード拡張** — Web ダッシュボード用のカスタム UI コンポーネント
...
NanoGemClaw には `plugins/` ディレクトリに 7 つのビルトインプラグインが付属しています:
...
| **memorization-service**    | Event Bus による自動会話要約          |               |  イベント駆動  |
...
| `WEBHOOK_EVENTS`     | `error,alert`              | Webhook をトリガーするイベント    |
| `ALERTS_ENABLED`     | `true`                     | メイングループへのエラー通知     |
| `CONTAINER_MAX_OUTPUT_SIZE` | `10485760`          | コンテナ最大出力サイズ (バイト)  |
| `SCHEDULER_CONCURRENCY` | 自動                    | 最大同時スケジュールコンテナ数   |
| `BACKUP_RETENTION_DAYS` | `7`                     | データベースバックアップ保持日数 |
| `HEALTH_CHECK_ENABLED` | `true`                   | ヘルスチェック HTTP サーバー有効化 |
...
- `/admin help` — 利用可能な管理コマンドを表示
- `/admin stats` — 稼働時間・メモリ使用量・トークン統計を表示
- `/admin groups` — 登録済み全グループとステータスを一覧表示
- `/admin tasks` — 全スケジュールタスクを一覧表示
- `/admin errors` — 最近エラーが発生したグループを表示
- `/admin report` — 日次使用レポートを生成
- `/admin language <lang>` — Bot インターフェースの言語を切り替え
- `/admin persona <name|list|set>` — Bot ペルソナを管理
- `/admin trigger <group> <on|off>` — @メンショントリガー要件を切り替え
- `/admin export <group>` — 会話履歴を Markdown でエクスポート
...
React + Vite + TailwindCSS の SPA で 12 モジュール構成:

| ページ            | 説明                                                                                |
| ----------------- | ----------------------------------------------------------------------------------- |
| **Overview**      | リアルタイムエージェントアクティビティ付きグループステータスカード                  |
| **Logs**          | レベルフィルタリング付きユニバーサルログストリーム                                  |
| **Activity Logs** | グループ別アクティビティ履歴とイベントタイムライン                                  |
...
| **Drive**         | Google Drive ファイルブラウザとドキュメントビューアー                                |
...
`HEALTH_CHECK_PORT` (デフォルト 8080) で軽量 HTTP サーバーが稼働:

- `GET /health` — システムヘルスステータス (healthy/degraded/unhealthy)
- `GET /ready` — オーケストレーター用レディネスプローブ
- `GET /metrics` — Prometheus 形式メトリクス

`HEALTH_CHECK_ENABLED=false` で無効化。
...
npm test                  # 全テスト実行 (Vitest, 41 ファイル, ~950 テスト)
...
- **STT が失敗する?** デフォルトプロバイダー（`gemini`）は追加の依存関係は不要です。`STT_PROVIDER=gcp` を使用する場合は `ffmpeg` をインストールしてください（`brew install ffmpeg`）。
