...
## 附錄 D：跨文件交叉驗證 — 與 gws CLI 分析合併路線圖（2026-03-05）

本附錄將本文件（ADK JS 分析）的建議與 [gws CLI 分析](./GOOGLE-WORKSPACE-CLI-ANALYSIS.md) 的建議進行交叉比對，產出最終合併的優先級路線圖。

### D.1 兩份分析的交叉驗證

| 項目 | gws 分析建議 | ADK 分析建議 | 是否一致？ | 合併判定 |
|------|-------------|-------------|-----------|---------|
| **MCP 整合** | P3（gws 作為 MCP server） | P1↑（MCP client bridge） | ✓ 互補 | **P2**：ADK 方案做 client bridge，gws 作為首個 server 測試目標 |
| **Tool hooks** | 未提及 | P1（beforeToolCall/afterToolCall） | — | **P1**：gws 的 prompt injection 掃描正好需要 afterToolCall hook |
| **Zod 驗證** | 未提及 | P1（搭配 MCP） | — | **P2**：與 MCP 一起做，MCP tools 大量受惠 |
| **PolicyEngine** | 未提及 | P2→P3↓ | — | **P3**：僅做 per-tool rate limiting，現有 9 層安全機制夠用 |
| **統一驗證** | P1 | 未提及 | — | **P1**：gws 分析獨有，零風險純重構 |
| **Prompt injection 掃描** | P2 | 未提及 | — | **P1**：可作為 afterToolCall hook 的首個實用案例 |
| **統一 Tool JSON** | P1 | 未提及 | — | **P1**：逐步遷移，為 hook 和 dashboard 打基礎 |
| **OS Keyring** | P3 | 未提及 | — | **P3**：維持，有 native 依賴考量 |
| **YAML Frontmatter** | P2→P3↓ | 未提及 | — | **P3**：功能已被 permission 系統覆蓋 |
| **State 分層** | 未提及 | P3→移除 | — | **移除**：現有系統已覆蓋 |
| **LLM Processors** | 未提及 | P3→移除 | — | **移除**：會破壞 context caching |
| **Agent-as-Tool** | 未提及 | P4 | — | **P4**：維持 |
| **Discovery-Driven** | 不建議 | 未提及 | — | **不做**：白做工最高 |
...
├── State 分層               ← 現有系統已覆蓋，強行抽象破壞型別安全
├── LLM Processors           ← 會破壞 context caching，需求可用現有 hooks 滿足
├── 完整 gws 替代現有 plugin  ← 會丟失 sync / hook 等業務邏輯
├── Persona Bundles          ← GEMINI.md per-group 已覆蓋
├── Multi-agent 編排         ← 延遲和成本與 Telegram 即時通訊場景衝突
└── Rust 重寫               ← 無必要
```
...
最大的白做工風險是 **Discovery-driven 動態生成**——NanoGemClaw 已有 `googleapis` + 手動 plugin 的深度整合，不需要覆蓋全部 Google API surface。
