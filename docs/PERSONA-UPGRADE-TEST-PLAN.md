...
**執行日期**: 2026-03-04 ~ 2026-03-05
**測試環境**: macOS, Dashboard (localhost:5173), API (localhost:3000), Playwright (Chrome)
**API 認證**: `x-access-code` header（DASHBOARD_API_KEY 為空時免認證）

### 進度總覽

| 測試項 | 狀態 | 備註 |
|--------|------|------|
| P1.1 15 個內建模板完整性 | ✅ | curl 驗證 15 personas, 所有 builtIn=true |
| P1.2 分類覆蓋驗證 | ✅ | 7 分類全部覆蓋，分布正確 |
| P1.3 System Prompt 長度規範 | ✅ | 全部在 150–400 字元範圍內 |
| P1.4 優先權鏈（Group Prompt > Persona > Default） | ✅ | Telegram 驗證：切換 persona 後回覆風格改變 |
| P2.1 GET /api/personas 回傳 builtIn 標記 | ✅ | 內建=true，自訂=false |
| P2.2 POST /api/personas 含 category | ✅ | 回傳 200 + key |
| P2.3 POST /api/personas category 驗證 | ✅ | 無效 category 回傳 400 |
| P2.4 POST /api/personas 重複 key 拒絕 | ✅ | 🐛 原回 200（覆蓋），已修正為 409 |
| P2.5 POST /api/personas 覆蓋內建拒絕 | ✅ | 🐛 原回 500，已修正為 409 |
| P2.6 DELETE /api/personas/:key | ✅ | 回傳 200 + success |
| P2.7 DELETE 內建 persona 拒絕 | ✅ | 🐛 原回 500，已修正為 400 |
| P2.8 向下相容：無 category 欄位 | ✅ | 成功建立，category 不存在 |
| P3.1 PersonaBrowser 卡片渲染 | ✅ | Playwright 驗證卡片 grid + badge + 按鈕 |
| P3.2 分類標籤過濾 | ✅ | 各分類過濾結果正確 |
| P3.3 搜尋功能 | ✅ | name/description 搜尋 + 清除按鈕 |
| P3.4 Apply Persona | ✅ | 套用後出現藍色選取框 + Active 按鈕 |
| P3.5 Preview Modal | ✅ | modal 顯示完整內容，Apply/Template 按鈕正常 |
| P3.6 已選取狀態指示 | ✅ | ring-2 + 勾選圖示 + Active 文字 |
| P4.1 Create Persona — 基本流程 | ✅ | 🐛 原建立後 UI 不刷新，已修正（refreshKey prop） |
| P4.2 Create Persona — 表單驗證 | ✅ | key 格式、必填欄位驗證正確 |
| P4.3 Create Persona — 從模板建立 | ✅ | template 下拉預填表單正確 |
| P4.4 Create Persona — 字數計數器 | ✅ | 即時計數，>2000 變琥珀色 |
| P4.5 Create Persona — Prompt 撰寫指南 | ✅ | 5 條提示，摺疊/展開正常 |
| P4.6 Create Persona — Preview Tab | ✅ | 預覽內容正確，切換保持表單狀態 |
| P4.7 Edit Persona | ✅ | 🐛 原缺 Edit 按鈕，已修正（加 onEdit prop + modal） |
| P4.8 Edit — Key 欄位鎖定 | ✅ | Playwright 驗證：key 欄位 disabled，值正確顯示 |
| P5.1 GroupDetailPage 版面整合 | ✅ | PersonaBrowser 全寬 + 3 欄 grid 正確 |
| P5.2 Persona 切換影響群組回覆 | ✅ | Telegram 驗證：Software Engineer persona 回覆技術問題含程式碼表格 |
| P5.3 舊 PersonaSelector 移除確認 | ✅ | 檔案已刪除，無引用 |
| P6.1 瀏覽器調整大小響應式 | ✅ | 375/768/1280px 三種寬度通過 |
| P6.2 大量自訂 persona 效能 | ⬜ | 未測試 |
| P6.3 API 錯誤處理 | ⬜ | 未測試 |
...
| P1 後端模板 | 4 | 0 | 0 |
| P2 API 端點 | 8 | 0 | 0 |
| P3 PersonaBrowser | 6 | 0 | 0 |
| P4 CreateEditModal | 8 | 0 | 0 |
| P5 整合 | 3 | 0 | 0 |
| P6 邊界情況 | 1 | 0 | 2 |
| **總計** | **30** | **0** | **2** |
...
| Bug | 嚴重度 | 修正檔案 | 說明 |
|-----|--------|---------|------|
| P2.4 重複 key 被接受（覆蓋） | High | `src/personas.ts` | `saveCustomPersona()` 缺少 custom key 存在性檢查，加入 `throw Error` |
| P2.5 覆蓋內建回 500 | Medium | `src/routes/groups.ts` | POST catch 區塊未區分錯誤類型，加入 `Cannot override` 檢查回 409 |
| P2.7 刪除內建回 500 | Medium | `src/routes/groups.ts` | DELETE catch 區塊未區分錯誤類型，加入 `Cannot delete` 檢查回 400 |
| P4.1 建立後 UI 不刷新 | Medium | `PersonaBrowser.tsx`, `GroupDetailPage.tsx` | 兩個獨立 `usePersonas()` hook 實例，加入 `refreshKey` prop 觸發內部 refetch |
| P4.7 Custom persona 缺 Edit 按鈕 | Low | `GroupDetailPage.tsx` | 未傳 `onEdit` prop，加入 `editingPersona` state + 回調 + modal |

### 未測試項目說明

| 項目 | 原因 |
|------|------|
| P6.2 大量效能 | 非關鍵路徑，可後續補測 |
| P6.3 API 錯誤處理 | 需模擬 API 斷線，非關鍵路徑 |
