...
<p align="center">
  由 <strong>Gemini</strong> 驱动，深度整合 <strong>Google 生态系统</strong>的个人 AI 助手。在容器中安全运行。轻量级，易于理解、定制和扩展。
</p>
...
| **架构**            | 单体                  | 模块化 monorepo（8 个包 + 7 个插件）               |
| **可扩展性**        | 硬编码                | 带生命周期钩子的插件系统                           |
| **Google 生态系统** | -                     | Drive、Calendar、Tasks、知识 RAG                   |
| **通知**            | -                     | Discord 日报/周报                                  |
...
- **模块化 Monorepo** — 8 个 npm workspace 包。可在自己的项目中单独使用各个包，也可部署完整技术栈。
...
- **Google Calendar（读写）** — 通过 Google Calendar API 创建、更新、删除日程及查询空闲时段。无 OAuth 时自动回退至 iCal 只读。
- **Google Tasks** — 完整 CRUD 操作，支持 NanoGemClaw 计划任务与 Google Tasks 的双向同步。
- **Google Drive** — 搜索文件、读取内容并生成摘要。支持 Docs、Sheets、PDF 和纯文本。
- **Drive 知识 RAG** — 两层检索：预索引 embedding 即时查询 + 实时 Drive 搜索扩大覆盖。可与 NotebookLM 共用同一知识文件夹。
- **Discord 报告** — 自动推送日报与周报至 Discord，采用色彩编码 embed 格式并附带控制面板链接。
...
│   ├── google-drive/         # Drive 文件搜索、读取与摘要
│   ├── google-tasks/         # Tasks CRUD 与双向同步
│   ├── google-calendar-rw/   # Calendar 读写（从 iCal 升级）
│   ├── drive-knowledge-rag/  # 两层 RAG（embedding + 实时搜索）
│   ├── discord-reporter/     # 日报与周报 Discord embed 推送
│   └── memorization-service/ # 自动对话摘要
├── app/               # 应用入口 — 将所有包连接在一起
...
| `@nanogemclaw/event-bus`  | 类型化 pub/sub 事件系统，供插件间通信 | 中       |
...
- `@Andy 帮我创建明天下午 3 点和 John 的会议`
- `@Andy 这周日历上有什么安排？`
- `@Andy 在 Google Tasks 新增一个「Review PR #42」任务`
- `@Andy 搜索 Drive 上的 Q4 预算电子表格`
- `@Andy 帮我摘要 Drive 上的项目提案文档`
- `@Andy 我的知识文档中有关于部署的说明吗？`
...
| `@nanogemclaw/event-bus`  | EventBus, NanoEventMap, 类型化 pub/sub singleton                                          |
...
npm test                  # 运行所有测试（Vitest，41 个文件，约 950 个测试）
...
- **Google OAuth 不工作？** 确认已设置 `GOOGLE_CLIENT_ID` 和 `GOOGLE_CLIENT_SECRET`。在 Google Cloud Console 中使用"桌面应用"类型。
- **Drive/Calendar/Tasks 没有响应？** 请先在控制面板设置 → Google 账户完成 OAuth 授权流程。
- **Discord 报告未发送？** 确认 `DISCORD_WEBHOOK_URL` 有效。可在控制面板设置中使用"发送测试"按钮。
