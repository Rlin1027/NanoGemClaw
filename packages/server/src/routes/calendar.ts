...
export function createCalendarRouter(deps: CalendarRouterDeps): Router {
    ...
    router.get('/calendar/configs', async (_req, res) => {
        try {
            const { getCalendarConfigs } = await import('../../../../src/google-calendar.js');
    ...
    router.post('/calendar/configs', async (req, res) => {
        try {
            ...
            const { saveCalendarConfig } = await import('../../../../src/google-calendar.js');
    ...
    router.delete('/calendar/configs', async (req, res) => {
        try {
            ...
            const { removeCalendarConfig } = await import('../../../../src/google-calendar.js');
    ...
    router.get('/calendar/events', async (req, res) => {
        try {
            const { getCalendarConfigs, fetchCalendarEvents } = await import('../../../../src/google-calendar.js');

            let days = 7;
            if (req.query.days) {
                const parsedDays = validateNumericParam(req.query.days as string, 'days');
                if (parsedDays === null) {
