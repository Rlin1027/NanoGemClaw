...
import { TELEGRAM } from '@nanogemclaw/core';
import { logger } from '@nanogemclaw/core/logger';
import { formatError } from '@nanogemclaw/core';

// ============================================================================
// Bot instance management (init pattern replaces state.js imports)
// ============================================================================

let botInstance: TelegramBot | null = null;
const typingIntervals: Map<string, NodeJS.Timeout> = new Map();

export function initTelegramHelpers(bot: TelegramBot): void {
  botInstance = bot;
}

function getBot(): TelegramBot {
  if (!botInstance) {
    throw new Error('TelegramHelpers not initialized — call initTelegramHelpers(bot) first');
  }
  return botInstance;
}

function setTypingInterval(chatId: string, interval: NodeJS.Timeout): void {
  typingIntervals.set(chatId, interval);
}

function clearTypingInterval(chatId: string): void {
